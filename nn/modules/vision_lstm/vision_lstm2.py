...
class ViLLayer(nn.Module):
    def __init__(self, dim, direction, expansion=2, qkv_block_size=4, proj_bias=True, norm_bias=True, conv_bias=True, conv_kernel_size=3, conv_kind="2d", init_weights="original-fixed", seqlens=None, num_blocks=15, gate_soft_cap=15.0, ffn_proj_factor=2.6667, ffn_round_up_to_multiple_of=64, weight_mode="fused", chunk_size=64):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # mLSTM processing
        h_tilde_state = self.mlstm_cell(q=q, k=k, v=v)  # (B, S, NH*DHHV)
...
class MatrixLSTMCell(nn.Module):
    def __init__(self, dim, num_heads, DHQK, DHHV, norm_bias=True, eps=1e-6, chunk_size=16, use_autocast=True, autocast_dtype=torch.float16, gate_soft_cap=15.0):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.DHQK = DHQK  # Dimension per head for queries and keys
        self.DHHV = DHHV  # Dimension per head for values
        self.use_autocast = use_autocast
        self.autocast_dtype = autocast_dtype
        self.gate_soft_cap = gate_soft_cap  # Cap value for soft capping

        # Define input dimension for ifgate: NH * DHQK (q) + NH * DHQK (k) + NH * DHHV (v)
        input_dim = num_heads * DHQK * 2 + num_heads * DHHV
        self.ifgate = nn.Linear(input_dim, 2 * num_heads)

        self.outnorm = MultiHeadLayerNorm(ndim=num_heads * DHHV, weight=True, bias=norm_bias, eps=1e-6)
        self.causal_mask_cache = {}
        self.chunk_size = chunk_size

        # CPU-compatible backend configuration (inference)
        ...
        # GPU-compatible (Triton) backend configuration (inference)
        ...
        # CPU-compatible backend configuration (training)
        ...
        # GPU-compatible (Triton) backend configuration (training)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        B, NH, S, _ = q.shape  # q: (B, NH, S, DHQK), k: (B, NH, S, DHQK), v: (B, NH, S, DHHV)
        dtype = q.dtype
        device = q.device
        ...
        # Reshape to (B, S, NH*DHQK) for q and k, (B, S, NH*DHHV) for v
        q_reshaped = q.transpose(1, 2).contiguous().view(B, S, -1)  # (B, S, NH*DHQK)
        k_reshaped = k.transpose(1, 2).contiguous().view(B, S, -1)  # (B, S, NH*DHQK)
        v_reshaped = v.transpose(1, 2).contiguous().view(B, S, -1)  # (B, S, NH*DHHV)

        # Concatenate for ifgate input
        if_gate_input = torch.cat([q_reshaped, k_reshaped, v_reshaped], dim=-1)  # (B, S, 2*NH*DHQK + NH*DHHV)
        ...
        h_state_norm = self.outnorm(h_state)  # (B, NH, S, DHHV)
        h_state_norm = h_state_norm.transpose(1, 2).reshape(B, S, -1)  # (B, S, NH*DHHV)
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.zeros_(self.ifgate.weight)  # Shape: (2 * num_heads, input_dim)
